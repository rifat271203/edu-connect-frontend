  ...
  <div class="flex-1 overflow-y-auto px-4 lg:px-6 py-8">
    <div class="max-w-4xl mx-auto">
      <div class="rounded-[14px] border border-[var(--line)] bg-[var(--surface)] p-6 sm:p-8 mb-6">
        ...
        <h2 class="font-display text-3xl leading-7 text-[var(--t1)] mb-2">Start your session</h2>
        <p class="text-sm text-[var(--t2)] leading-relaxed max-w-2xl">
          {{ description }}
        </p>
      </div>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <button
          ...
          class="text-left rounded-xl border border-[var(--line)] bg-[var(--surface2)] p-4 hover:border-[var(--line-gold)] hover:bg-[var(--surface3)] transition-all duration-200 group"
        >
          <div class="flex items-start gap-3">
            <span class="text-[var(--gold)] mt-0.5" v-html="prompt.icon" />
            <div class="min-w-0">
              <p class="text-sm font-semibold text-[var(--t1)] group-hover:text-[var(--gold2)] transition-colors">{{ prompt.text }}</p>
              <p class="mono-label text-[11px] text-[var(--t3)] mt-1">{{ prompt.tag }}</p>
            </div>
          </div>
        </button>
      </div>
